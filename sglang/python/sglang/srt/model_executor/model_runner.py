...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def __init__(
        ...
        kv_cache_data: Optional[Dict[str, torch.Tensor]] = None,
        ...
        self.kv_cache_data = kv_cache_data
        kv_cache_socket_path = os.environ.get("SGLANG_KV_CACHE_SOCKET_PATH")
        if self.kv_cache_data is None and kv_cache_socket_path:
            from sglang.srt.mem_cache.kv_cache_server import get_kv_cache_from_server

            # Supported formats:
            # 1) "/tmp/kv_cache_{}.sock" -> format with gpu_id
            # 2) "/tmp/a.sock,/tmp/b.sock" -> choose by gpu range
            # 3) "/tmp/kv_cache.sock" -> direct path
            if "{}" in kv_cache_socket_path:
                socket_path_to_use = kv_cache_socket_path.format(gpu_id)
            elif "," in kv_cache_socket_path:
                socket_paths = [path.strip() for path in kv_cache_socket_path.split(",")]
                min_gpu_id = int(os.environ.get("SGLANG_MIN_GPU_ID", "0"))
                socket_index = (gpu_id - min_gpu_id) // tp_size
                if socket_index < len(socket_paths):
                    socket_path_to_use = socket_paths[socket_index]
                    logger.info(
                        "GPU %s (tp_rank %s) uses KV cache socket[%s]: %s",
                        gpu_id,
                        tp_rank,
                        socket_index,
                        socket_path_to_use,
                    ...
                    raise ValueError(
                        f"GPU {gpu_id} computed socket_index {socket_index} exceeds "
                        f"number of socket paths {len(socket_paths)}. Available paths: "
                        f"{socket_paths}, min_gpu_id: {min_gpu_id}, tp_size: {tp_size}"
                    )
            else:
                socket_path_to_use = kv_cache_socket_path
            self.kv_cache_data = get_kv_cache_from_server(
                socket_path_to_use, tp_rank=tp_rank
            )
    ...
    def load_model(self):
        ...
        self.load_config = LoadConfig(
            ...
            weight_port=self.server_args.weight_load_port,
        )
        if self.device == "cpu":
        ...
        weight_region = (
            empty_context()
            if (
                self.server_args.skeleton_worker
                and self.server_args.load_format == LoadFormat.WEIGHT_DAEMON
            )
            else self.memory_saver_adapter.region(
