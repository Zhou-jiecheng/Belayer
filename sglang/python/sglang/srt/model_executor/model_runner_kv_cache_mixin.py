...
class ModelRunnerKVCacheMixin:
    ...
    def init_memory_pool(self: ModelRunner, total_gpu_memory: int):
        ...
        if self.kv_cache_data is not None and "max_total_num_tokens" in self.kv_cache_data:
            self.max_total_num_tokens = self.kv_cache_data["max_total_num_tokens"]
            logger.info(
                "Using max_total_num_tokens from KV cache server: %s",
        ...
        if (
            max_num_reqs is None
            and self.kv_cache_data is not None
            and "max_num_reqs" in self.kv_cache_data
        ):
            max_num_reqs = self.kv_cache_data["max_num_reqs"]
            logger.info("Using max_num_reqs from KV cache server: %s", max_num_reqs)
        ...
        if self.req_to_token_pool is None:
            ...
            if self.server_args.disaggregation_mode == "decode":
                ...
                req_to_token = (
                    self.kv_cache_data["req_to_token"] if self.kv_cache_data else None
                )
        ...
        if self.server_args.attention_backend == "ascend":
            ...
            if self.is_hybrid_swa:
                ...
                if is_float4_e2m1fn_x2(self.kv_cache_dtype):
                    ...
                    k_buffer = (
                        self.kv_cache_data["k_buffer"] if self.kv_cache_data else None
                    )
                    v_buffer = (
                        self.kv_cache_data["v_buffer"] if self.kv_cache_data else None
                    )
