...
class Qwen3ForCausalLM(nn.Module):
    ...
    def get_weights_by_name(
        ...
        """Get the weights of the parameter by its HF-style name."""
        try:
            ...
            if mapped_shard_id is not None:
                if mapped_shard_id in ["q", "k", "v"]:
                    ...
                    if mapped_shard_id == 0:
                        offset = 0
                    else:
                        offset = slice_size
                    weight = param.data.narrow(0, offset, slice_size)
                else:
                    weight = param.data
            else:
                weight = param.data
            ...
            logger.exception("Error getting weights by name %s in Qwen3ForCausalLM", name)
            return None
