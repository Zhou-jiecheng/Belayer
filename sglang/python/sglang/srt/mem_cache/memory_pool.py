...
class MHATokenToKVPool(KVCache):
    ...
    def __init__(
        ...
        k_buffer: Optional[List[torch.Tensor]] = None,
        v_buffer: Optional[List[torch.Tensor]] = None,
    ):
        super().__init__(
            size,
            page_size,
        ...
        if self.k_buffer is not None and len(self.k_buffer) != self.layer_num:
            if len(self.k_buffer) >= self.start_layer + self.layer_num:
                self.k_buffer = self.k_buffer[
                    self.start_layer : self.start_layer + self.layer_num
                ]
            else:
                raise ValueError(
                    f"Invalid shared k_buffer size {len(self.k_buffer)} for layer_num={self.layer_num}"
                )
        if self.v_buffer is not None and len(self.v_buffer) != self.layer_num:
            if len(self.v_buffer) >= self.start_layer + self.layer_num:
                self.v_buffer = self.v_buffer[
                    self.start_layer : self.start_layer + self.layer_num
                ]
            else:
                raise ValueError(
                    f"Invalid shared v_buffer size {len(self.v_buffer)} for layer_num={self.layer_num}"
                )
