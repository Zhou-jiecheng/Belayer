...
class Scheduler(
    ...
    def __init__(
        ...
        self.init_shadow_worker_support()
    ...
    def init_shadow_worker_support(self):
        self.shadow_standby_timeout_s = float(
            os.environ.get("SGLANG_SHADOW_HEARTBEAT_TIMEOUT", "2.0")
        )
        self._shadow_standby_done = not self.server_args.skeleton_worker
        self.heartbeat_executor = ThreadPoolExecutor(max_workers=1)
        self._heartbeat_future = None
        self.heartbeat_file = f"/tmp/sglang_heartbeat_{self.gpu_id}_{self.tp_rank}.log"

        if self.server_args.skeleton_worker:
            logger.info(
                "Skeleton worker enabled, entering standby until heartbeat timeout on %s",
                self.heartbeat_file,
            )
            self.memory_saver_adapter.pause(GPU_MEMORY_TYPE_CUDA_GRAPH)

    def _write_heartbeat_task(self):
        try:
    ...
    def _update_heartbeat(self):
        if self._heartbeat_future is not None and not self._heartbeat_future.done():
            return
        self._heartbeat_future = self.heartbeat_executor.submit(
            self._write_heartbeat_task
        )

    def _maybe_wait_for_shadow_takeover(self):
        if self._shadow_standby_done:
            return

        self._shadow_standby_done = True
        logger.info("Skeleton worker standby phase started.")
        process = psutil.Process()
        original_affinity = None
        try:
            original_affinity = process.cpu_affinity()
            if original_affinity:
                process.cpu_affinity([original_affinity[0]])
            if hasattr(psutil, "IOPRIO_CLASS_IDLE"):
                process.ionice(psutil.IOPRIO_CLASS_IDLE)
        except Exception as e:
            logger.debug("Failed to set standby resource limits: %s", e)

        while True:
            try:
                if os.path.exists(self.heartbeat_file):
                    last_heartbeat = os.path.getmtime(self.heartbeat_file)
                    if time.time() - last_heartbeat > self.shadow_standby_timeout_s:
                        logger.warning(
                            "Heartbeat timeout detected (%.2fs). Skeleton worker taking over.",
                            self.shadow_standby_timeout_s,
                        )
                        break
            except OSError:
                pass
        ...
        try:
            ...
            if hasattr(psutil, "IOPRIO_CLASS_NONE"):
                process.ionice(psutil.IOPRIO_CLASS_NONE)
        except Exception as e:
            logger.debug("Failed to restore process priority/affinity: %s", e)

        self.memory_saver_adapter.resume(GPU_MEMORY_TYPE_CUDA_GRAPH)
        self.flush_cache()
        logger.info("Skeleton worker switched to active mode.")
    ...
    def init_overlap(self):
        ...
        # This queue is also referenced by cache-flush/no-request checks that can
        # run before entering event_loop_overlap (e.g. skeleton-worker takeover).
    ...
    def event_loop_normal(self):
        ...
        self._maybe_wait_for_shadow_takeover()
        while True:
