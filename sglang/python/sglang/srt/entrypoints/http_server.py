...
async def health_generate(request: Request) -> Response:
    ...
    # Skeleton workers can stay in standby waiting for heartbeat takeover,
    # so no detokenizer response may be observed yet. Keep health check green
    # during this bootstrap window to avoid false negatives in handover tests.
    if (
        _global_state.tokenizer_manager.server_args.skeleton_worker
        and _global_state.tokenizer_manager.last_receive_tstamp <= 0
    ):
