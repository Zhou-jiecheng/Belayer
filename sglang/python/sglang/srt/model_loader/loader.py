...
def set_model_parameter(model: nn.Module, param_name: str, new_tensor: torch.Tensor):
    """Replace a model parameter tensor while preserving custom attributes."""

    name_parts = param_name.split(".")
    parent_module: Any = model
    for part in name_parts[:-1]:
        if hasattr(parent_module, part):
            ...
            raise AttributeError(f"Cannot resolve module path for parameter '{param_name}'")

    final_attr_name = name_parts[-1]
    original_param = getattr(parent_module, final_attr_name)
    new_param = nn.Parameter(new_tensor, requires_grad=original_param.requires_grad)

    # Preserve only explicitly attached custom attributes.
    # Iterating over dir(original_param) may trigger expensive tensor properties
    # (for example `.T`), which is unnecessary and can cause side effects.
    custom_attrs = {}
    for attr_name, attr_value in getattr(original_param, "__dict__", {}).items():
        if attr_name.startswith("_") or attr_name == "weight_loader":
            continue
        if callable(attr_value):
            continue
        custom_attrs[attr_name] = attr_value
    set_weight_attrs(new_param, custom_attrs)

    if hasattr(original_param, "weight_loader"):
        new_param.weight_loader = original_param.weight_loader
    else:
        new_param.weight_loader = default_weight_loader
...
class WeightDaemonModelLoader(BaseModelLoader):
    """Model loader that acquires weights from checkpoint-engine via CUDA IPC."""
    ...
    def __init__(self, load_config: LoadConfig):
        ...
        if load_config.weight_port is None:
            raise ValueError(
                "--weight_load_port must be provided for --load-format weight_deamon."
            )
        ...
        self.socket.connect(f"tcp://127.0.0.1:{load_config.weight_port}")

    def download_model(self, model_config: ModelConfig) -> None:
        pass

    def initialize_weights_from_handles(self, model: nn.Module):
        rank = get_tensor_model_parallel_rank()
        self.socket.send_pyobj({"command": "get_all_handles", "rank": rank})
        response = self.socket.recv_pyobj()
        if response.get("status") != "success":
            raise RuntimeError(
                f"Failed to get tensors from weight daemon: {response.get('message')}"
            )

        handles = response.get("handles", {})
        expected_params = set(dict(model.named_parameters()).keys())
        loaded_names = set()
        skipped_errors: List[Tuple[str, str]] = []
        unknown_handle_names = []
        for name, ipc_handle in handles.items():
            if name not in expected_params:
                unknown_handle_names.append(name)
                continue
            try:
                func, args = ipc_handle
                args = list(args)
                if self.device_id is not None and len(args) > 6:
                    # Remap source device id to local logical cuda index.
                    args[6] = self.device_id
                tensor = func(*args)
                set_model_parameter(model, name, tensor)
                loaded_names.add(name)
            except Exception as e:
                skipped_errors.append((name, str(e)))

        missing_params = sorted(expected_params - loaded_names)
        logger.info(
            "Loaded %d/%d parameters from weight daemon via CUDA IPC (received %d handles, rank=%d).",
            len(loaded_names),
            len(expected_params),
            len(handles),
            rank,
        )
        if unknown_handle_names:
            logger.warning(
                "Weight daemon returned %d unknown handles. Sample: %s",
                len(unknown_handle_names),
                unknown_handle_names[:8],
            )
        if skipped_errors:
            logger.warning(
                "Failed to apply %d weight handles. Sample: %s",
                len(skipped_errors),
                skipped_errors[:5],
            )
        if missing_params:
            logger.warning(
                "Missing %d model parameters after weight daemon load. Sample: %s",
                len(missing_params),
                missing_params[:8],
            )

        if get_bool_env_var("SGLANG_STRICT_WEIGHT_DAEMON_LOAD"):
            if skipped_errors or missing_params:
                raise RuntimeError(
                    "Strict weight daemon load check failed. "
                    f"missing_params={len(missing_params)}, failed_handles={len(skipped_errors)}"
                )
    ...
    def load_model(
        ...
        *,
        ...
        self.device_id = device_config.gpu_id
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            self.initialize_weights_from_handles(model)
            post_load_weights(model, model_config)
...
class RemoteInstanceModelLoader(BaseModelLoader):
    ...
    def load_model_from_remote_instance_by_nccl(
        ...
        build_group_success, build_group_message = client.build_group(
            gpu_id=device_config.gpu_id,
            tp_rank=load_config.tp_rank,
            instance_ip=instance_ip,
        )
        if not build_group_success:
            raise RuntimeError(build_group_message)
...
def get_model_loader(
    ...
    if load_config.load_format == LoadFormat.WEIGHT_DAEMON:
        return WeightDaemonModelLoader(load_config)
