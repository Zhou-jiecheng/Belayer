...
def _get_remote_instance_request_timeout_sec() -> float:
    return max(0.1, float(os.getenv("SLIME_REMOTE_INSTANCE_REQUEST_TIMEOUT_SEC", "2")))
...
def trigger_init_weights_send_group_for_remote_instance_request(
    ...
    try:
        requests.post(
            ...
            timeout=_get_remote_instance_request_timeout_sec(),
        )
