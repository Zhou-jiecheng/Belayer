...
def _invert_deprecated_bool_env(new_name: str, old_name: str):
    ...
    try:
        old_value = EnvBool(False).parse(os.environ[old_name])
    except ValueError:
        warnings.warn(
            f'Invalid value for deprecated environment variable {old_name}: '
            f'"{os.environ[old_name]}". Keeping original value for {new_name}.'
        )
        os.environ[new_name] = os.environ[old_name]
        return

    os.environ[new_name] = str(not old_value).lower()
