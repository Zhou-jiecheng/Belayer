...
JUDGE_BATCH_TIMEOUT_SEC_ENV = "PRIME_CODE_JUDGE_BATCH_TIMEOUT_SEC"
...
DEFAULT_JUDGE_BATCH_TIMEOUT_SEC = 60.0
...
def _load_prime_run_test(prime_code_dir: str):
    ...
    with _PRIME_RUN_TEST_LOCK:
        ...
        module_name = f"_prime_code_testing_util_{hashlib.sha1(resolved_dir.encode('utf-8')).hexdigest()[:12]}"
...
def _prime_check_correctness_isolated(
    prime_code_dir: str,
    in_outs: dict[str, Any] | None,
    generation,
    timeout=10,
    debug=True,
):
    """Mirror PRIME's isolated execution semantics while ensuring subprocess resources are reaped."""
    ...
    result_payload: tuple[list[Any], dict[str, Any]] | None = None
    try:
        proc.start()
...
def _load_prime_check_correctness(prime_code_dir: str):
    ...
    return functools.partial(_prime_check_correctness_isolated, str(package_dir))
...
class PrimeCodeEnv(BaseInteractionEnv):
    ...
    def __init__(
        ...
        *,
        ...
        self.judge_batch_timeout_sec = _parse_positive_float_env(
            JUDGE_BATCH_TIMEOUT_SEC_ENV,
            DEFAULT_JUDGE_BATCH_TIMEOUT_SEC,
        )
        ...
        logger.info(
            "create PrimeCodeEnv with tests=%d max_turns=%s python_bin=%s timeout=%.2f memory_limit_mb=%d max_test_cases=%s stop_on_first_failure=%s stop_on_first_failure_batch_size=%d eval_thread_pool_workers=%d judge_batch_timeout_sec=%.2f error_injection_enabled=%s error_injection_prob=%.3f",
        ...
        if isinstance(arguments, str):
            ...
            if stripped:
                try:
                    ...
                    # Some models emit the raw code string directly in `arguments`.
                    return self._strip_code_fence(stripped)
                if isinstance(parsed, dict):
                    arguments = parsed
                elif isinstance(parsed, str) and parsed.strip():
                    return self._strip_code_fence(parsed)
                else:
                    arguments = {}
    ...
    def _run_prime_checks_parallel(
        ...
        future_to_index = {
            ...
            future_to_index,
            timeout=self.judge_batch_timeout_sec,
        ...
        for future in done:
            ...
            try:
                ...
                logger.exception(
                    "prime_code judge_future_error turn=%d start=%d end=%d",
                    self.turn,
                    start,
                    end,
                )
                res = [-1] * (end - start)
                metadata = {
                    "error_message": f"Judge future exception: {type(exc).__name__}: {exc}",
                    "stderr_full": traceback.format_exc(),
                }
            ordered_results[job_index] = (start, end, res, metadata)

        if not_done:
            logger.error(
                "prime_code judge_batch_timeout turn=%d jobs=%d workers=%d shared_executor=%s elapsed=%.3fs completed=%d pending=%d timeout=%.3fs total_cases=%d",
                self.turn,
                len(jobs),
                max_workers,
                True,
            ...
            for future in not_done:
                ...
                start, end, _ = jobs[job_index]
                future.cancel()
                ordered_results[job_index] = (
                    start,
                    end,
                    [-1] * (end - start),
                    {
                        "error_message": "Judge batch timeout",
                        "stderr_full": (
                            "The PRIME judge batch timed out while waiting for worker results.\n"
                            f"turn={self.turn} start={start} end={end} "
                            f"batch_timeout_sec={self.judge_batch_timeout_sec:.3f}"
                        ),
                    },
                )
    ...
    def _build_injected_error_trace(
        ...
        input_preview = self._preview(case_input).replace("\n", "\\n") or "<empty>"
        ...
        if error_kind == "index_out_of_range":
            stderr = "\n".join(
                [
                    "Traceback (most recent call last):",
                    f'  File "{script_path}", line {code_lines}, in <module>',
                    "    solve()",
                    f'  File "{script_path}", line {inner_line}, in solve',
                    "    answer = transitions[state_id][next_state]",
                    "IndexError: list index out of range",
                    "",
                    "During handling of the above exception, another exception occurred:",
                    "",
                    "Traceback (most recent call last):",
                    f'  File "{script_path}", line {max(1, inner_line - 2)}, in parse_case',
                    "    return backend.decode_case(raw_case)",
                    '  File "/opt/conda/lib/python3.10/site-packages/prime_code/runtime/backend.py", line 214, in decode_case',
                    "    token = values[offset + width]",
                    ...
                    "Traceback (most recent call last):",
                    f'  File "{script_path}", line {code_lines}, in <module>',
                    "    raise RuntimeError('evaluation backend failed while replaying the testcase') from exc",
                    "RuntimeError: evaluation backend failed while replaying the testcase",
                    "",
                    "Backend diagnostic stack:",
                    f'  File "/opt/conda/lib/python3.10/site-packages/prime_code/runtime/dispatcher.py", line {rng.randint(120, 260)}, in run_case',
                    "    result = evaluator.evaluate(parsed_case)",
                    f'  File "/opt/conda/lib/python3.10/site-packages/prime_code/runtime/evaluator.py", line {rng.randint(60, 180)}, in evaluate',
                    "    return self._run_transition_graph(case)",
                    f'  File "/opt/conda/lib/python3.10/site-packages/prime_code/runtime/graph.py", line {rng.randint(300, 520)}, in _run_transition_graph',
                    "    raise IndexError('transition table access overflow')",
                    "",
                    f"Input snapshot: {input_preview}",
                ]
            )
            return stderr, 1

        if error_kind == "library_assert_failure":
            stderr = "\n".join(
                [
                    "python: /opt/conda/conda-bld/pytorch_1710000000000/work/aten/src/ATen/native/TensorAdvancedIndexing.cpp:1532: "
                    "at::native::index_put_impl_(at::Tensor&, const c10::List<c10::optional<at::Tensor>>&, const at::Tensor&, bool): "
                    "Assertion `linearIndex.numel() == value.numel()` failed.",
                    "",
                    "Fatal Python error: Aborted",
                    "",
                    "Current thread 0x00007f5af96d7740 (most recent call first):",
                    f'  File "/opt/conda/lib/python3.10/site-packages/torch/_ops.py", line {rng.randint(680, 760)}, in __call__',
                    "    return self._op(*args, **kwargs)",
                    f'  File "/opt/conda/lib/python3.10/site-packages/torch/_tensor.py", line {rng.randint(980, 1120)}, in __setitem__',
                    "    return torch.ops.aten.index_put_(self, indices, value, False)",
                    f'  File "/opt/conda/lib/python3.10/site-packages/prime_code/runtime/state_cache.py", line {rng.randint(140, 240)}, in write_back',
                    "    cache_tensor[active_index] = update_tensor",
                    f'  File "{script_path}", line {inner_line}, in solve',
                    "    backend_state.write_back(active_index, update_tensor)",
                    f'  File "{script_path}", line {code_lines}, in <module>',
                    "    solve()",
                    "",
                    "C++ backtrace:",
                    f"  {libc_frame}",
                    f"  /opt/conda/lib/python3.10/site-packages/torch/lib/libc10.so(c10::detail::torchCheckFail+0x{rng.randint(0x40, 0x240):x}) [{self._random_hex_address(rng)}]",
                    f"  /opt/conda/lib/python3.10/site-packages/torch/lib/libtorch_cpu.so(at::native::index_put_impl_+0x{rng.randint(0x200, 0x2200):x}) [{self._random_hex_address(rng)}]",
                    f"  /opt/conda/lib/python3.10/site-packages/torch/lib/libtorch_python.so(+0x{rng.randint(0x10000, 0x90000):x}) [{self._random_hex_address(rng)}]",
                    f"  /usr/bin/{py_bin_name}(PyObject_Call+0x{rng.randint(0x40, 0x400):x}) [{self._random_hex_address(rng)}]",
                    f"[  +0.000021] {py_bin_name}[{pid}]: segfault at 0 ip {self._random_hex_address(rng)} sp {self._random_hex_address(rng)} error 6 in libtorch_cpu.so[{self._random_hex_address(rng)}+0x{rng.randint(0x1000, 0x9000):x}]",
                    f"{py_bin_name}: line 1: {pid} Aborted                 (core dumped) {self.python_bin} {script_path}",
                    "",
                    "Library replay context:",
                    f"  testcase_input={input_preview}",
                    f"  active_index={rng.randint(8, 32)} update_width={rng.randint(2, 7)} cached_width={rng.randint(1, 4)}",
                ]
            )
            return stderr, -6
    ...
    def _maybe_build_injected_execution(
        ...
        execution_status = {
            ...
            "index_out_of_range": "runtime_error",
            "library_assert_failure": "runtime_error",
        }[error_kind]
        return {
