...
MODEL_PATH = os.environ.get(
    ...
    "/mnt/shared-storage-user/ailab-sys/zhoujiecheng/projs/models/Qwen2.5-7B-Instruct",
)
...
def execute():
    ...
    run_name = f"{model_name}_prime_code_multi_turn_w_format_reward_{'fault_inject_' + INJECTION_PROB if IF_INJECTION else 'no_fault_injection'}"
    checkpoint_dir = (
        "/mnt/shared-storage-user/ailab-sys/zhoujiecheng/projs/robust_rl/OpenClaw-RL/"
        f"slime/checkpoints/prime_code_multi_turn_format_reward/{run_name}"
    )
    ...
    rollout_args = (
        ...
        f"--rollout-batch-size {1 if MODE == 'debug_one_sample' else 4 if MODE == 'debug_minimal' else 16} "
        ...
        "--rollout-top-p 1 "
        f"--global-batch-size {1 if MODE == 'debug_one_sample' else 8 if MODE == 'debug_minimal' else 128} "
        "--save-debug-rollout-data /mnt/shared-storage-user/ailab-sys/zhoujiecheng/projs/robust_rl/OpenClaw-RL/slime/debug_data/prime_code_format_reward/Qwen2.5-7B-Instruct-multi-turn-fault-inject-0-0-5-max-turn-1/data_{rollout_id}.pt "
    )
    ...
    sglang_args = (
        ...
        f"{'--sglang-disable-cuda-graph ' if MODE != 'normal' else ''}"
