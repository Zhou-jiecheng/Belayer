...
def train(args):
    assert args.debug_rollout_only, (
        "train_async_rollout_only.py requires --debug-rollout-only so argument "
        "normalization and GPU placement match rollout-only mode."
    )
    assert not args.colocate, "Colocation is not supported for async rollout-only mode."
    ...
    try:
        for rollout_id in range(args.start_rollout_id, args.num_rollout):
            if rollout_data_next_future is not None:
                # Rollout data is generated and converted inside RolloutManager;
                # here we only synchronize to preserve async overlap behavior.
