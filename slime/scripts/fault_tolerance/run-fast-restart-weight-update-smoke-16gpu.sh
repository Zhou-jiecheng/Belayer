...
WORKDIR="${WORKDIR:-${SLIME_ROOT}/workdir/slime_fast_restart_weight_update_smoke_16gpu_${RUN_ID}}"
...
for idx in "${!ROLLOUT_GPU_SETS[@]}"; do
    ...
    KV_SOCKET_PATHS+=("/tmp/kv_cache_weight_update_smoke_16gpu_${idx}.sock")
...
start_head_and_submit_job() {
    ...
    echo "CI fault injection engine indices: ${CI_FAULT_INJECTION_ENGINE_INDEX}"
...
EOF
    ...
    echo "FAST RESTART + WEIGHT UPDATE 16-GPU SMOKE TEST PASSED"
