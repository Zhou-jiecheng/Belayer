...
# 2. checkpoint policy enabled, but online scheduler disabled, and
...
# Keep static concurrency / no online-scheduler / no fault-injection semantics
# even if the caller inherited those env vars elsewhere.
...
echo "[swe-static-checkpoint] SWE_ENABLE_ONLINE_ENV_DOCKER_SCHEDULER=${SWE_ENABLE_ONLINE_ENV_DOCKER_SCHEDULER}"
