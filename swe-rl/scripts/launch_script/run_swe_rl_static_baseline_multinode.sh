...
# 2. checkpoint policy disabled, and
# 3. otherwise aligned training/topology defaults with the adaptive 32-GPU launcher.
...
export SAVE_CKPT="${SAVE_CKPT:-${SWE_RL_DIR}/../export/ckpt/swe-rl-static-baseline-256}"
...
export SWE_SCHED_PRESERVE_PROMPT_ORDER=0
...
export SWE_SCHED_DOCKER_CREATE_MAX_CONCURRENT="${SWE_SCHED_DOCKER_CREATE_MAX_CONCURRENT:-${CONTROL_PLANE_CAPACITY_LIMIT}}"
...
# Force the baseline to stay off the adaptive-checkpoint / online-scheduler /
# fault-injection paths even if the caller inherited those env vars elsewhere.
export SWE_CHECKPOINT_POLICY=never
export SWE_ENABLE_ONLINE_ENV_DOCKER_SCHEDULER=0
...
COORD_ROOT="${SWE_RL_DIR}/../export/.swe_rl_static_baseline_coord_${MASTER_ADDR//./_}_${COORD_JOB_KEY}"
RUN_ID="${RUN_ID:-}"
...
WORKDIR="${WORKDIR:-${SWE_RL_DIR}/../export/swe_rl_static_baseline_256_${RUN_ID}}"
...
wait_for_head_completion() {
    echo "[swe-static-baseline] Waiting for head completion marker: ${DONE_FILE}"
    ...
    echo "[swe-static-baseline] Head finished with exit code ${head_exit_code}"
    exit "${head_exit_code}"
}
...
start_env_pool_server() {
    ...
    python3 -m swe_env_pool_server \
        ...
        > "${pool_log}" 2>&1 &
    SWE_POOL_PID="$!"
    echo "[swe-static-baseline] SWE env pool server PID=${SWE_POOL_PID}, log=${pool_log}"
...
echo "[swe-static-baseline] MASTER_ADDR=${MASTER_ADDR}"
echo "[swe-static-baseline] CURRENT_NODE_IP=${CURRENT_NODE_IP}"
echo "[swe-static-baseline] MODEL_PATH=${MODEL_PATH}"
echo "[swe-static-baseline] PROMPT_DATA=${PROMPT_DATA}"
echo "[swe-static-baseline] ACTOR_NODE_COUNT=${ACTOR_NODE_COUNT}"
echo "[swe-static-baseline] ROLLOUT_NODE_COUNT=${ROLLOUT_NODE_COUNT}"
echo "[swe-static-baseline] ACTOR_GPU_SET=${ACTOR_GPU_SET}"
echo "[swe-static-baseline] ROLLOUT_GPU_SET_CSV=${ROLLOUT_GPU_SET_CSV}"
echo "[swe-static-baseline] TENSOR_MODEL_PARALLEL_SIZE=${TENSOR_MODEL_PARALLEL_SIZE}"
echo "[swe-static-baseline] ROLLOUT_TP_SIZE=${ROLLOUT_TP_SIZE}"
echo "[swe-static-baseline] ROLLOUT_TOTAL_GPU_COUNT=${ROLLOUT_TOTAL_GPU_COUNT}"
echo "[swe-static-baseline] STATIC_MAX_CONCURRENCY=${STATIC_MAX_CONCURRENCY}"
echo "[swe-static-baseline] CONTROL_PLANE_CONCURRENCY=${CONTROL_PLANE_CONCURRENCY}"
echo "[swe-static-baseline] SWE_STATIC_CAPACITY_HEADROOM=${SWE_STATIC_CAPACITY_HEADROOM}"
echo "[swe-static-baseline] SWE_POOL_MAX_TOTAL_LEASES=${SWE_POOL_MAX_TOTAL_LEASES}"
echo "[swe-static-baseline] SWE_MAX_CONTAINERS_PER_NODE=${SWE_MAX_CONTAINERS_PER_NODE}"
echo "[swe-static-baseline] SWE_MAX_CONCURRENT=${SWE_MAX_CONCURRENT}"
echo "[swe-static-baseline] SWE_POOL_MAX_CONCURRENT_ALLOCATES=${SWE_POOL_MAX_CONCURRENT_ALLOCATES}"
echo "[swe-static-baseline] SWE_MAX_CONCURRENT_DOCKER_CREATE=${SWE_MAX_CONCURRENT_DOCKER_CREATE}"
echo "[swe-static-baseline] SWE_ENABLE_ONLINE_ENV_DOCKER_SCHEDULER=${SWE_ENABLE_ONLINE_ENV_DOCKER_SCHEDULER}"
echo "[swe-static-baseline] SWE_CHECKPOINT_POLICY=${SWE_CHECKPOINT_POLICY}"
echo "[swe-static-baseline] SWE_FAULT_INJECTION_ENABLE=${SWE_FAULT_INJECTION_ENABLE}"
echo "[swe-static-baseline] NUM_ROLLOUT=${NUM_ROLLOUT}"
echo "[swe-static-baseline] ROLLOUT_BATCH_SIZE=${ROLLOUT_BATCH_SIZE}"
echo "[swe-static-baseline] N_SAMPLES_PER_PROMPT=${N_SAMPLES_PER_PROMPT}"
echo "[swe-static-baseline] OVER_SAMPLING_BATCH_SIZE=${OVER_SAMPLING_BATCH_SIZE}"
echo "[swe-static-baseline] TARGET_TOTAL_SAMPLES=${TARGET_TOTAL_SAMPLES}"
echo "[swe-static-baseline] SLIME_ENABLE_ROLLING_OVERSAMPLING_REFILL=${SLIME_ENABLE_ROLLING_OVERSAMPLING_REFILL}"

start_head_and_submit_job() {
    echo "[swe-static-baseline] Head node: ${CURRENT_NODE_IP}"
    echo "[swe-static-baseline] Ray head: ${RAY_HEAD_IP}:${RAY_HEAD_PORT}"
    echo "[swe-static-baseline] Workdir: ${WORKDIR}"
    echo "[swe-static-baseline] Topology: actor ${ACTOR_NODE_COUNT}x${ACTOR_GPU_COUNT}GPU + rollout ${ROLLOUT_TOTAL_ENGINE_COUNT}x${ROLLOUT_TP_SIZE}GPU across ${ROLLOUT_NODE_COUNT} rollout node(s)"

    ray stop --force >/dev/null 2>&1 || true
    echo "[swe-static-baseline] Starting Ray head"
    ray start --head \
    ...
    echo "[swe-static-baseline] Waiting ${RAY_WORKER_JOIN_WAIT_SEC}s for worker nodes to join Ray..."
    ...
    ray_job_submission_id="${RAY_JOB_SUBMISSION_ID:-swe_rl_static_baseline_256_${RUN_ID}}"
    ...
    {
        echo
        echo "[swe-static-baseline] ===== ray job logs: ${ray_job_submission_id} ====="
...
start_worker_node() {
    echo "[swe-static-baseline] Non-head node: ${CURRENT_NODE_IP}"
    echo "[swe-static-baseline] MASTER_ADDR: ${MASTER_ADDR}"

    ray stop --force >/dev/null 2>&1 || true
    echo "[swe-static-baseline] Starting Ray worker"
    until ray start \
        --address="${MASTER_ADDR}:${RAY_HEAD_PORT}" \
        --node-ip-address "${CURRENT_NODE_IP}" \
        --num-gpus "${NUM_GPUS_PER_NODE}" \
        --disable-usage-stats >/dev/null; do
        echo "[swe-static-baseline] Ray head ${MASTER_ADDR}:${RAY_HEAD_PORT} not ready yet, retrying worker join..."
        ray stop --force >/dev/null 2>&1 || true
        sleep 2
    done
    ...
    echo "[swe-static-baseline] Current node role: ${self_role}"
